...
- No external auth or database layer
...
- No server-side persistence beyond in-memory runtime state
...
4) The rendered preview is optional and only fetched when the user clicks "Show rendered document." Preview bytes stay in memory so they can be fetched again until the server restarts.
...
- `frontend/app.js` - UI flow, lazy preview loading
...
- No history layer: avoids retaining user documents or thumbnails.
